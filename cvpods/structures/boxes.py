...
class Boxes:
    ...
    def nonempty(self, threshold: int = 1) -> torch.Tensor:
        """
...
def box_cxcywh_to_xyxy(x):
    ...
    b = [(x_c - 0.5 * w), (y_c - 0.5 * h), (x_c + 0.5 * w + 0.001), (y_c + 0.5 * h + 0.001)]
...
def union_box(boxes1, boxes2):
    """
    obtains the union of two boxes list
    Args:
        boxes1:
        boxes2:
    ...
    assert len(boxes1) == len(boxes2)
    boxes1, boxes2 = boxes1.tensor, boxes2.tensor
    rb = torch.max(boxes1[:, 2:], boxes2[:, 2:])  # [M,2]
    lt = torch.min(boxes1[:, :2], boxes2[:, :2])  # [M,2]
