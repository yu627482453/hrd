...
class OneStageEncDecVRD(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.counterfactual = cfg.MODEL.REL_DETR.COUNTERFACTUAL
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training:
            if self.counterfactual:
                crel_hs, cext_inter_feats, crel_decoder_out_res = self.relation_decoder(
                    self.rel_input_proj(src),
                    mask,
                    self.rel_query_embed.weight,
                    rel_src_pos_embed,
                ...
                pred_rel_vec_counterfactual = self.rel_vector_embed(crel_hs.feature)  # batch_size, num_queries, 4
                pred_rel_vec = pred_rel_vec - pred_rel_vec_counterfactual
