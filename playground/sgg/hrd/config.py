...
_config_dict = dict(
    ...
    MODEL=dict(
        ...
        ENTITY_NUM_CLASSES=150,
        ENTITY_NUM_QUERIES=100,
        FREEZE_ANCHOR_BOX_CENTERS=True,
        ENTITY_AUX_LOSS=True,
        PIXEL_MEAN=[123.675, 116.280, 103.530],
        PIXEL_STD=[58.395, 57.120, 57.375],
        RELATION_EMBED_DIM=256,
        RELATION_NUM_QUERY=180,
        RELATION_NUM_CLASS=50,
        ...
        WEIGHTS_FIXED=[
            "backbone",
            "entity_transformer.encoder",
            "entity_transformer.decoder",
        ],

        #####
        # TEST_WEIGHTS="/root/project/sgtr/cvpods/playground/sgg/hgtr/VG-sgtr/000000.pth",
        TEST_WEIGHTS="/root/project/sgtr/cvpods/playground/sgg/hgtr/vg_model/model_detr_2.pth",
        # WEIGHTS="/root/project/sgtr/cvpods/playground/sgg/hgtr/VG-sgtr/000000.pth",
        WEIGHTS="/root/project/sgtr/cvpods/playground/sgg/hgtr/vg_model/model_detr_2.pth",
        RESNETS=dict(
            DEPTH=101,
            OUT_FEATURES=["res5"],
        ),
        ...
        ENTITY_POSITION_EMBEDDING=dict(
            NUM_POS_FEATS=128,
            TEMPERATURE=10000,
            NORMALIZE=True
        ),
        RELATION_POSITION_EMBEDDING=dict(
            NUM_POS_FEATS=128,
            TEMPERATURE=10000,
            NORMALIZE=True
        ),
        RELATION_TRANSFORMER=dict(
            ENCODER=dict(
                EMBED_DIM=256,
                NUM_HEADS=8,
                ATTN_DROPOUT=0.0,
        ...
        REL_DETR=dict(  # relationship DETR
            ...
            ENTITIES_AWARE_HEAD=dict(
                ...
                # use the entity prediction from predicate entity sub decoder rather than graph assembling
                ENTITIES_INDEXING=True,  # training time Graph assembling

                USE_ENTITIES_INDEXING_RANKING=False,
                INDEXING_TYPE="rule_base",
                # the entity  matching method for graph assembeling; other options: feat_att rule_base
    ...
    SOLVER=dict(
        ...
        IMS_PER_BATCH=6,  # 四卡时候的batchsize
        IMS_PER_DEVICE=6,
...
class HgtrRelDetrConfig(OneStageRelDetrBASEConfig):
    def __init__(self):
        super(HgtrRelDetrConfig, self).__init__()
        self._register_configuration(_config_dict)


config = HgtrRelDetrConfig()
